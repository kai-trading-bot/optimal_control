...
class Gradients:
    ...
    def swish(self, x: float) -> float:
        return self.beta * Functions().swish(x) + \
               (Functions().sigmoid(x * self.beta) * (1 - self.beta * Functions().swish(x)))
    ...
    def gelu(self, x: float, alpha: float = 0.01) -> float:
        ...
        np.exp(-(56146134542243 * x * ((6444110634811895 * x ** 2) / 144115188075855872 + 1)) / 35184372088832) + 1) + \
               (x * np.exp(-(56146134542243 * x * ((6444110634811895 * x ** 2) / 144115188075855872 +
                                                   1)) / 35184372088832) *
                ((1085435708121742813248709141455 * x ** 2) / 5070602400912917605986812821504 +
                 56146134542243 / 35184372088832)) / (np.exp(
                   -(56146134542243 * x * ((6444110634811895 * x ** 2) / 144115188075855872
                                           + 1)) / 35184372088832) + 1) ** 2
    ...
    for t in range(1, tmax):
        ...
        for i in range(n):
            if ((i != p) & ((theta_old[i] - theta_old[p]) != 0) & (np.abs(theta_old[i] - theta_old[p]) != 1) & (
                ...
                if theta_old[p] > theta_old[i]:
                    theta_new[i] = theta_old[i] + (
                    w * t_weight * (func(theta_old[i] - theta_old[p], b=beta)) * (func(theta_old[i], b=beta)))
                else:
                    theta_new[i] = theta_old[i] - (
                    w * t_weight * (func(theta_old[i] - theta_old[p], b=beta)) * (func(theta_old[i], b=beta)))
            else:
